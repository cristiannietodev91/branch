...
const createTaller = (req, res, next) => {
  try {
    ...
    tallerDAO.create(taller, function (error, taller) {
      if (error) {
        console.error(
          "Error al realizar la transaccion de crear taller:::>",
          "error ::>",
          error
        );
        if (error.errors) {
        ...
        if (taller) {
          ...
          return res
            ...
            .json({ error: "No se creo el taller" });
        }
      }
    });
  } catch (error) {
  ...
  try {
    ...
    if (IdTaller) {
      tallerDAO.update(IdTaller, taller, function (error, taller) {
        if (error) {
          console.error(
            "Error al realizar la transaccion de actualizar taller:::>",
            "error ::>",
            error.message
          );
          return res
          ...
          if (taller) {
            ...
            return res
              .status(HttpStatus.OK)
              .json({ error: "No se actualizo el taller" });
          }
        }
      });
    } else {
...
const findTallerById = (req, res, next) => {
  try {
    ...
    tallerDAO.getById(IdTaller, function (error, taller) {
      if (error) {
        console.error(
          "Error al realizar la transaccion de get taller:::>",
          "error ::>",
          error
        );
        if (error.errors) {
