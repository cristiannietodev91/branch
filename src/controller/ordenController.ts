...
const createOrden = (req: Request, res: Response) => {
  try {
    ...
    if (orden.IdCita) {
      citaAdapter.findCitaByIdCita(orden.IdCita)?.then(cita => {
        if (cita) {
          ...
          if (textoSms != "") {
            if(cita.vehiculo && cita.vehiculo.usuarios){
              ...
              notificacionAdapter.crearNotificacion(notificacion)?.then((notification => {
          ...
          ordenDAO.create(ordenDB)?.then(orden => {
            if (orden) {
              if (orden.IdEtapa) {
                if (orden.IdEtapa == 7) {
                ...
                citaDAO.update(cita.IdCita, cita)?.then(cita => {
                  if (cita) {
...
const updateOrden = (req: Request, res: Response) => {
  try {
    ...
    if (IdOrdenTrabajo) {
      ...
      ordenDAO.update({IdOrdenTrabajo}, ordenDb)?.then(orden => {
        if (orden) {
          ordenDAO.getById(IdOrdenTrabajo)?.then(orden => {
            /*if (orden) {
              debug(
...
const getAllOrdenesByIdTaller = (req: Request, res: Response) => {
  try {
    ...
    ordenDAO.findAllByFilter({ IdTaller: IdTaller }, {}, {})?.then(ordenes => {
      if (ordenes) {
...
const getOrdenById = (req: Request, res: Response) => {
  try {
    ...
    ordenDAO.getById(IdOrden)?.then(orden => {
      if (orden) {
        return res.status(HttpStatus.OK).json(orden);
      } else {
...
const getAllOrdenesByIdTallerAndFilter = (req: Request, res: Response) => {
  try {
    ...
    if (filter) {
      ...
      ordenAdapter.getOrdenesByIdTallerActivas(IdTaller)?.then(ordenes => {
        if (ordenes) {
          res.status(HttpStatus.OK).json(ordenes);
        }
      }).catch(error => {
...
const countOrdenesByIdTaller = (req: Request, res: Response) => {
  try {
    ...
    ordenAdapter.countOrdenesByIdTaller(IdTaller)?.then(citas => {
      if (citas) {
